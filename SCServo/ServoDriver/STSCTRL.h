...
// === NEW FUNCTION: ENABLE MULTI-TURN ===
void setMultiTurn(byte InputID, bool enable){
  st.unLockEprom(InputID);
  delay(1000);
  
  if(enable){
    // Set Limits to 0 = UNLIMITED (Multi-Turn Mode)
    // Works for both Type 1 and Type 2
    st.writeWord(InputID, 9, 0);  // Min Angle Limit
    st.writeWord(InputID, 11, 0); // Max Angle Limit
  }
  else {
    // Restore Single-Turn Limits (0-4095)
    st.writeWord(InputID, 9, 0);     // Min Angle Limit
    st.writeWord(InputID, 11, 4095); // Max Angle Limit
  }
...
void setMode(byte InputID, byte InputMode){
  ...
  if(InputMode == 0){
    if(SERVO_TYPE_SELECT == 1){
      //st.writeWord(InputID, 11, 4095); // <--- This forces single-turn! Hence have removed it 
